...
const GroupChatScreen = ({ navigation, route }) => {
  ...
  const sendImageMessage = async (imageUri) => {
    ...
    try {
      const sentMessage = await groupService.sendImageMessage(group.id, imageUri);
  ...
  const loadMessages = useCallback(async () => {
    try {
      const data = await groupService.getMessages(group.id);
      setMessages(data);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (subscriptionRef.current) {
        groupService.unsubscribe(subscriptionRef.current);
      }
    };
  }, [group.id, loadMessages, currentUser]);
  ...
  const handleSend = async () => {
    ...
    try {
      const sentMessage = await groupService.sendMessage(group.id, messageText);
      setMessages(prev => prev.map(m => m.id === tempMessage.id ? sentMessage : m));
    } catch (error) {
  ...
  // Calcular dimensões da imagem mantendo aspect ratio
  const getImageStyle = (imageUrl) => {
    ...
    if (!dims) {
      ...
      // Retornar tamanho padrão enquanto carrega
  ...
  const renderMessage = ({ item, index }) => {
    ...
    const showSender = !isOwnMessage && (index === 0 || messages[index - 1]?.sender?.id !== item.sender?.id);
    ...
    return (
      <View>
        ...
        <View style={[styles.messageContainer, isOwnMessage ? styles.ownMessage : styles.otherMessage]}>
          ...
          {!isOwnMessage && !showSender && <View style={styles.avatarSpacer} />}
      ...
      <View style={styles.header}>
        ...
        >
          <View style={styles.headerAvatar}>
            {group.image_url ? (
              <Image source={{ uri: group.image_url }} style={styles.headerAvatarImage} />
            ) : (
              <Ionicons name="people" size={20} color={colors.botanical.sage} />
            )}
          </View>
          <View>
            <Text style={styles.headerName} numberOfLines={1}>{group.name}</Text>
            <Text style={styles.headerStatus}>{group.member_count} membros</Text>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('GroupDetails', { group })}
        >
          <Ionicons name="information-circle-outline" size={24} color={colors.botanical.dark} />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: colors.botanical.base },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  headerAvatar: {
    ...
    backgroundColor: colors.botanical.sand, justifyContent: 'center', alignItems: 'center',
  },
  headerAvatarImage: { width: 40, height: 40, borderRadius: 20 },
  headerName: { fontSize: 16, fontWeight: '600', color: colors.botanical.dark },
  headerStatus: { fontSize: 12, color: colors.botanical.sage },
  ...
  emptyText: { fontSize: 16, fontWeight: '600', color: colors.botanical.dark, marginTop: spacing.md },
  emptySubtext: { fontSize: 14, color: colors.botanical.sage, marginTop: spacing.xs },
  dateSeparator: { alignItems: 'center', marginVertical: spacing.md },
  dateText: {
    fontSize: 12, color: colors.botanical.sage, backgroundColor: colors.botanical.sage + '20',
  ...
  messageAvatar: { width: 28, height: 28, borderRadius: 14, marginRight: spacing.xs },
  avatarSpacer: { width: 28, marginRight: spacing.xs },
  messageContent: { maxWidth: '75%' },
  senderName: { fontSize: 12, fontWeight: '600', color: colors.botanical.sage, marginBottom: 2, marginLeft: spacing.xs },
  messageBubble: { padding: spacing.sm, borderRadius: borderRadius.md },
  ownBubble: { backgroundColor: colors.botanical.clay, borderBottomRightRadius: 4 },
  otherBubble: { backgroundColor: colors.ui.background, borderBottomLeftRadius: 4 },
  messageText: { fontSize: 15, lineHeight: 20 },
  ownMessageText: { color: colors.botanical.base },
  otherMessageText: { color: colors.botanical.dark },
  messageTime: { fontSize: 10, marginTop: 4 },
  ownMessageTime: { color: colors.botanical.base + 'AA', textAlign: 'right' },
  otherMessageTime: { color: colors.botanical.sage },
  inputContainer: {
    flexDirection: 'row', alignItems: 'flex-end', padding: spacing.md,
    backgroundColor: colors.ui.background, borderTopWidth: 1, borderTopColor: 'rgba(46, 74, 61, 0.1)',
  },
  attachButton: {
    width: 44, height: 44, justifyContent: 'center', alignItems: 'center', marginRight: spacing.xs,
  },
  input: {
    flex: 1, minHeight: 40, maxHeight: 100, backgroundColor: colors.botanical.base,
    borderRadius: borderRadius.md, paddingHorizontal: spacing.md, paddingVertical: spacing.sm,
    fontSize: 15, color: colors.botanical.dark, marginRight: spacing.sm,
  },
  sendButton: {
    width: 44, height: 44, borderRadius: 22, backgroundColor: colors.botanical.clay,
    justifyContent: 'center', alignItems: 'center',
  },
  sendButtonDisabled: { backgroundColor: colors.botanical.sage, opacity: 0.5 },
  imageBubble: {
    padding: spacing.xs, borderRadius: borderRadius.md, overflow: 'hidden',
  },
  messageImage: {
  ...
  uploadingOverlay: {
    ...
    justifyContent: 'center', alignItems: 'center', borderRadius: borderRadius.sm,
  },
  imageTime: {
